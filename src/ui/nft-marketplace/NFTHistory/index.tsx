...
const NFTHistory = ({ historyList }: NFTHistoryProps) => (
  <div className="flex flex-col bg-white dark:bg-indigo rounded-md w-full 4xl:max-w-nftCard overflow-auto shadow-md pb-[46px] pt-[19px]">
    ...
    <div className="flex justify-center flex-col px-[18px]">
      {historyList.map(({ image, title, author, price, timeAgo }, index) => (
        <div
          key={index}
          className="flex gap-8 justify-between p-5 rounded-md hover:shadow-md hover:dark:bg-indigo-light"
        >
          <div className="flex items-center gap-4">
            <Image
              ...
              width={66}
              height={66}
              className="rounded-xl w-[66px] h-[66px] object-cover hidden xs:block"
            />
